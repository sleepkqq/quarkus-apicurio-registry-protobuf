...
```
ru.meenity:apicurio-registry-protobuf:1.0.0-SNAPSHOT
```

### Gradle (consumer, e.g. meenity-social)

```kotlin
// libs.versions.toml
// apicurio-registry-protobuf = { group = "ru.meenity", name = "apicurio-registry-protobuf", version = "1.0.0" }
...
|---|---|
| `quarkus.apicurio.registry.protobuf.index-dependencies` | `groupId:artifactId` list of schema-library jars to Jandex-index so their message classes are discovered. Use for libraries (e.g. `ru.meenity:meenity-kafka-kt-schemas`) that ship no Jandex index. |
